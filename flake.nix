{
  ...
  outputs =
    ...
    let
      ...
      forAllSystems =
        fn: nixpkgs.lib.genAttrs (import systems) (system: withLib fn nixpkgs.legacyPackages.${system});
    in
    {
      lib = forAllSystems (
        pkgs: transpire: rec {
          ...
          inherit (import ./lib/image.nix { inherit (pkgs) lib; }) isImage imageType;
      ...
      packages = forAllSystems (
        pkgs: transpire: { docs = transpire.buildDocs { }; } // import ./generated-openapi-pkgs.nix pkgs
      );
      ...
      apps = forAllSystems (
        pkgs: transpire: {
          generate-openapi-pkgs = {
            type = "app";
            program = toString (
              pkgs.writers.writeBash "generate-openapi-pkgs" ''
